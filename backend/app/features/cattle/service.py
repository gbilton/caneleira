...
class CattleService:
    ...
    def create(self, data: CattleCreate) -> Cattle:
        ...
        if exists:
            raise HTTPException(
                status_code=400,
                detail=f"Cattle with identifier {data.identifier} already exists",
            )
    ...
    def get_by_id(self, cattle_id: UUID) -> CattleRead:
        ...
        if not cattle:
            raise HTTPException(
                status_code=404, detail=f"Cattle id {cattle_id} not found"
            )
...
class WeightHistoryService:
    ...
    def delete(self, cattle_id: UUID, weight_history_record_id: UUID) -> None:
        weight_history_record = self.repository.get_by_id(
            cattle_id, weight_history_record_id
        )
        if not weight_history_record:
            raise HTTPException(
                status_code=404,
                detail=f"Weight history record id {weight_history_record_id} for cattle id {cattle_id} not found",
            )
