...
from .schema import CattleCreate, CattleUpdate, CattleResponse, WeightHistoryCreate, WeightHistoryResponse
...
@router.post("/{cattle_id}/weight-history", response_model=WeightHistoryResponse, status_code=status.HTTP_201_CREATED)
def create_weight_history_record(data: WeightHistoryCreate, service: WeightHistoryService = Depends(get_weight_history_service)):
    return service.create(data.cattle_id, data.weight, data.measured_at)

@router.get("/{cattle_id}/weight-history", response_model=List[WeightHistoryResponse])
def get_weight_history(cattle_id: UUID, service: WeightHistoryService = Depends(get_weight_history_service)):
    return service.get_all(cattle_id)
@router.delete("/{cattle_id}/weight-history/{weight_history_record_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_weight_history(cattle_id: UUID, weight_history_record_id: UUID, service: WeightHistoryService = Depends(get_weight_history_service)):
