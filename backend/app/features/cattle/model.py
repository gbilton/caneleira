...
class Cattle(Base, BaseColumnsMixin):
    ...
    herd_id: Mapped[UUID | None] = mapped_column(
        SA_UUID(as_uuid=True), ForeignKey("herds.id"), nullable=True
    )

    herd = relationship("Herd", back_populates="cattle")
    weight_history = relationship(
        "WeightHistory", back_populates="cattle", cascade="all, delete-orphan"
    )
...
class WeightHistory(Base, BaseColumnsMixin):
    ...
    cattle_id: Mapped[UUID] = mapped_column(
        SA_UUID(as_uuid=True), ForeignKey("cattle.id"), nullable=False
    )
