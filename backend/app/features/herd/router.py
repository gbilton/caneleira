...
@router.post("", response_model=HerdResponse, status_code=status.HTTP_201_CREATED)
def create_herd(data: HerdCreate, service: HerdService = Depends(get_herd_service)):
...
def update_herd(herd_id: UUID, data: HerdUpdate, service: HerdService = Depends(get_herd_service)):
