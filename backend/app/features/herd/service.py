...
class HerdService:
    ...
    def create(self, data: HerdCreate) -> Herd:
        ...
        if exists:
            raise HTTPException(status_code=400, detail=f"Herd with name {data.name} already exists")
        return self.repository.create(data)
    
    def get_by_id(self, herd_id: UUID) -> HerdRead:
        herd = self.repository.get_by_id(herd_id)
        if not herd:
            raise HTTPException(status_code=404, detail=f"Herd id {herd_id} not found")
